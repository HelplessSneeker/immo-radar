...
function mieteZelle(daten: PortfolioZeileDaten): string {
  ...
  if (vergleich.eigeneMieteM2 === undefined) {
    return '<td class="num"><span class="unter-markt">leerstehend</span></td>';
  }
  const eigene = `${nfEur2.format(vergleich.eigeneMieteM2)} €/m²`;
  if (vergleich.miete === undefined) {
    return `<td class="num">${eigene}<span class="sub">Markt: zu wenige Vergleiche (&lt; ${MIN_VERGLEICHSOBJEKTE})</span></td>`;
  }
  ...
  const zeilen = daten.zeilen
    .map(({ objekt, vergleich }) => {
      return `      <tr>
        ...
        <td class="num">${nfTage.format(objekt.flaecheM2)} m²<span class="sub">${nfTage.format(objekt.zimmer)} Zi.${objekt.baujahr ? ` · Bj. ${objekt.baujahr}` : ''}</span></td>
        <td class="num">${nfEur0.format(objekt.kaufpreis)} €<span class="sub">${objekt.flaecheM2 > 0 ? `${nfEur0.format(objekt.kaufpreis / objekt.flaecheM2)} €/m²` : ''}</span></td>
