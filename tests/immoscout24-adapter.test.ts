...
describe('ImmoScout24Adapter', () => {
  ...
  it('wirft ImmoScout24Fehler bei HTTP-Fehlern und Netzwerkproblemen', async () => {
    const instant = { maxVersuche: 1, basisPauseMs: 0, maxPauseMs: 0, warte: async () => {} };
    const blockiert = (async () => new Response('blocked', { status: 403 })) as typeof fetch;
    await expect(new ImmoScout24Adapter(blockiert, 0, instant).fetch(KAUF_URL)).rejects.toThrow(/HTTP 403/);

    const offline = (async () => {
      throw new TypeError('fetch failed');
    }) as typeof fetch;
    await expect(new ImmoScout24Adapter(offline, 0, instant).fetch(KAUF_URL)).rejects.toThrow(ImmoScout24Fehler);
  });
  ...
  it('wiederholt transiente 5xx-Antworten und liefert danach das Ergebnis', async () => {
    ...
    const flaky = (async () => {
      ...
      return new Response(macheSeite(ids(5, 0), 5), { status: 200 });
    }) as typeof fetch;
    const adapter = new ImmoScout24Adapter(flaky, 0, {
      maxVersuche: 3,
      basisPauseMs: 0,
      maxPauseMs: 0,
      warte: async () => {},
  ...
  it('wiederholt Netzwerkfehler bis zum Erfolg', async () => {
    ...
    const flaky = (async () => {
      ...
      return new Response(macheSeite(ids(3, 0), 3), { status: 200 });
    }) as typeof fetch;
    const adapter = new ImmoScout24Adapter(flaky, 0, {
      maxVersuche: 3,
      basisPauseMs: 0,
  ...
  it('wiederholt persistente 4xx-Fehler nicht', async () => {
    ...
    const adapter = new ImmoScout24Adapter(blockiert, 0, {
      maxVersuche: 3,
      basisPauseMs: 0,
      maxPauseMs: 0,
      warte: async () => {},
